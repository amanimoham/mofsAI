...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const inputData = JSON.stringify({ csv_text: body.csv_text });

  return new Promise<NextResponse>((resolve) => {
    const child = spawn(PYTHON, [...PY_ARGS, 'analyze_dataset.py'], {
      cwd: PROJECT_ROOT,
    ...
    child.on('close', (code) => {
      if (code !== 0) {
        try {
          ...
          resolve(NextResponse.json({ api_status: 'error', error: parsed.error || stderr || 'Analyze failed' }, { status: 500 }));
        } catch {
          resolve(NextResponse.json({ api_status: 'error', error: stderr || stdout || 'Analyze failed' }, { status: 500 }));
        }
        return;
      }
      try {

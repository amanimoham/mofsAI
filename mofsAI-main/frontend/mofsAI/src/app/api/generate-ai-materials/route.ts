...
export async function POST(request: Request): Promise<NextResponse> {
  let body: {
    ...
    n_generate?: number;
    top_k?: number;
    vae?: { hidden_dim?: number; latent_dim?: number; lr?: number; epochs?: number; batch_size?: number };
  };
  try {
  ...
  const inputData = JSON.stringify({
    ...
    n_generate: body.n_generate ?? 200,
    top_k: body.top_k ?? 10,
    vae: body.vae ?? {},
  });

  return new Promise<NextResponse>((resolve) => {
    const child = spawn(PYTHON, [...PY_ARGS, 'generate_ai_materials.py'], {
      cwd: PROJECT_ROOT,
    ...
    child.on('close', (code) => {
      if (code !== 0) {
        try {
          ...
          resolve(NextResponse.json({ api_status: 'error', error: parsed.error || stderr || 'Generation failed' }, { status: 500 }));
        } catch {
          resolve(NextResponse.json({ api_status: 'error', error: stderr || stdout || 'Generation failed' }, { status: 500 }));
        }
        return;
      }
      try {
